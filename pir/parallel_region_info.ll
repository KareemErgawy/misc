; ModuleID = '<stdin>'
source_filename = "../llvm/test/PIR/parallel_region_info.ll"
...
define void @nested() {
entry:
  call void (%ident_t*, i32, void (i32*, i32*, ...)*, ...) @__kmpc_fork_call(%ident_t* @0, i32 0, void (i32*, i32*, ...)* @nested.entry)
  br label %join
...
; Function Attrs: noreturn nounwind uwtable
define internal void @nested.entry(i32* noalias %.global_tid., i32* noalias %.bound_tid., ...) #0 {
entry:
  %.global_tid..addr = alloca i32*, align 8
  %.bound_tid..addr = alloca i32*, align 8
  %.omp.sections.lb. = alloca i32, align 4
...
.omp.sections.case:                               ; preds = %omp.inner.for.body
  call void @nested.entry.forked.outer()
  br label %.omp.sections.exit

.omp.sections.case1:                              ; preds = %omp.inner.for.body
  call void @nested.entry.cont.outer()
  br label %.omp.sections.exit
...
define void @nested.entry.forked.outer() {
forked.outer:
  call void @foo()
  call void (%ident_t*, i32, void (i32*, i32*, ...)*, ...) @__kmpc_fork_call(%ident_t* @0, i32 0, void (i32*, i32*, ...)* @nested.entry.forked.outer.forked.outer)
  br label %forked.outer.end

forked.outer.end:                                 ; preds = %forked.outer
  br label %exit

exit:                                             ; preds = %forked.outer.end
  ret void
}

define void @nested.entry.cont.outer() {
cont.outer:
  call void @foo()
  br label %exit

exit:                                             ; preds = %cont.outer
  ret void
}

; Function Attrs: noreturn nounwind uwtable
define internal void @nested.entry.forked.outer.forked.outer(i32* noalias %.global_tid., i32* noalias %.bound_tid., ...) #0 {
entry:
  %.global_tid..addr = alloca i32*, align 8
  %.bound_tid..addr = alloca i32*, align 8
  %.omp.sections.lb. = alloca i32, align 4
...
.omp.sections.case:                               ; preds = %omp.inner.for.body
  call void @nested.entry.forked.outer.forked.outer.forked.inner()
  br label %.omp.sections.exit

.omp.sections.case1:                              ; preds = %omp.inner.for.body
  call void @nested.entry.forked.outer.forked.outer.cont.inner()
  br label %.omp.sections.exit
...
define void @nested.entry.forked.outer.forked.outer.forked.inner() {
forked.inner:
  call void @foo()
  br label %forked.inner.body

forked.inner.body:                                ; preds = %forked.inner
  call void @foo()
  br label %exit

exit:                                             ; preds = %forked.inner.body
  ret void
}

define void @nested.entry.forked.outer.forked.outer.cont.inner() {
cont.inner:
  call void @foo()
  br label %exit

exit:                                             ; preds = %cont.inner
  ret void
}
